...
- **`test`/`assembleDebug` hijau TIDAK membuktikan build release aman.** Unit test (`./gradlew test`) berjalan di atas bytecode debug yang belum melewati R8, dan `assembleDebug` tidak mengaktifkan `isMinifyEnabled`. Bug yang hanya muncul akibat shrinking/obfuscation (contoh nyata: `TypeToken<List<X>>() {}` milik `PairedDeviceStore`/`MacroStore` yang di-crash R8 walau `CoreStateTest` hijau — lihat `docs/troubleshooting.md` #11) hanya bisa ditemukan dengan `./gradlew :app:assembleRelease` lalu install nyata ke device. Anggap `assembleRelease` + acceptance manual sebagai level bukti terpisah, bukan tergantikan oleh `test`/`assembleDebug` yang hijau.
